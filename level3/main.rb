...
class Drivy
  ...
  class Rental
    ...
    def commission
      commission = {}
      commission[:insurance_fee] = (price * 0.3 * 0.5).to_i
      commission[:assistance_fee] = (100 * duration).to_i
      commission[:drivy_fee] = (price * 0.3).to_i - commission[:insurance_fee] - commission[:assistance_fee]

      commission
    end

    def for_json
      { id: @id, price: price.to_i, commission: commission }
    end
  end
end
